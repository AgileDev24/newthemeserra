...
if (frameExists("Body")) {
width = subModalResize("FrameBody", "W", width);
height = subModalResize("FrameBody", "H", height);
}
...
function frameExists(frameID) {
    for (var i = 0; i < parent.frames.length; i++) {
        if (parent.frames[i].name == frameID) {
            return true;
        }
    }
    return false;
...
function subModalResize(frameID, typ, val) {
    var frameSize = getFrameSize(frameID);
    var comp;

    if (typ == "W") { comp = frameSize.width; }
    else { comp = frameSize.height; }
